...
void UITask()
{
    // if(Interactive_data->flash_flag == 1){
    //     My_UI_init();
    //     // Interactive_data->flash_flag == 0;
    // }
    if(Interactive_data->flash_flag == 1 && ui_flag == 0)
    {
        /* code */
        MyUIInit();
        ui_flag = 1;
...
static Graph_Data_t UI_Bullet_speed[3]; 
static String_Data_t UI_State_sta[6];  // 机器人状态,静态只需画一次
static String_Data_t UI_State_dyn[6];  // 机器人状态,动态先add才能change
static uint32_t shoot_line_location[10] = {540, 960, 490, 515, 540};
...
void MyUIInit()
{
    ...
    // 绘制发射基准线，浅改下参数
    UILineDraw(&UI_shoot_line[0], "sl0", UI_Graph_ADD, 7, UI_Color_White, 3, 945, shoot_line_location[0], 975, shoot_line_location[0]);
    UILineDraw(&UI_shoot_line[1], "sl1", UI_Graph_ADD, 7, UI_Color_White, 3, shoot_line_location[1], 515, shoot_line_location[1], 565);

    
    UILineDraw(&UI_shoot_line[2], "sl2", UI_Graph_ADD, 7, UI_Color_Yellow, 2, 935, shoot_line_location[2], 985, shoot_line_location[2]);
    UILineDraw(&UI_shoot_line[3], "sl3", UI_Graph_ADD, 7, UI_Color_Yellow, 2, 910, shoot_line_location[3], 1010, shoot_line_location[3]);
    ...
    UICharDraw(&UI_State_sta[3], "ss3", UI_Graph_ADD, 8, UI_Color_Black, 15, 2, 150, 600, "frict:");
    UICharRefresh(&referee_recv_info->referee_id, UI_State_sta[3]);
    UICharDraw(&UI_State_sta[4], "ss4", UI_Graph_ADD, 8, UI_Color_Purplish_red, 15, 2, 150, 550, "bullet:");
    ...
    UICharDraw(&UI_State_dyn[3], "sd3", UI_Graph_ADD, 8, UI_Color_Black, 15, 2, 270, 600, "off");
    UICharRefresh(&referee_recv_info->referee_id, UI_State_dyn[3]);
    UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_ADD, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 2000);
    UIGraphRefresh(&referee_recv_info->referee_id, 1, UI_Bullet_speed[1]);
    // CharDraw(&UI_State_dyn[4], "sd4", UI_Graph_ADD, 8, UI_Color_Pink, 15, 2, 270, 550, "null");
    // Char_ReFresh(&referee_recv_info->referee_id, UI_State_dyn[4]);
    // Char_Draw(&UI_State_dyn[4], "sd4", UI_Graph_ADD, 8, UI_Color_Purplish_red, 15, 2, 270, 550, Interactive_data->Bullet_Speed);
...
static void MyUIRefresh(referee_info_t *referee_recv_info, Referee_Interactive_info_t *_Interactive_data)
{
    ...
    {
        UICharDraw(&UI_State_dyn[2], "sd2", UI_Graph_Change, 8, UI_Color_Orange, 15, 2, 270, 650, _Interactive_data->shoot_mode == SHOOT_ON ? "on " : "off");
        UICharRefresh(&referee_recv_info->referee_id, UI_State_dyn[2]);
    ...
    {
        UICharDraw(&UI_State_dyn[3], "sd3", UI_Graph_Change, 8, UI_Color_Black, 15, 2, 270, 600, _Interactive_data->friction_mode == FRICTION_ON ? "on " : "off");
        UICharRefresh(&referee_recv_info->referee_id, UI_State_dyn[3]);
        _Interactive_data->Referee_Interactive_Flag.friction_flag = 0;
    }
    if (_Interactive_data->Referee_Interactive_Flag.friction_measure_flag == 1)
    {
        switch (_Interactive_data->Bullet_Speed)
        {
            case BIG_AMU_10:
            {
                UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_Change, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 10 * 1000);
                break;
            }
            case BIG_AMU_12:
            {
                UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_Change, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 12 * 1000);
                break;
            }
            case BIG_AMU_14:
            {
                UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_Change, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 14 * 1000);
                break;
            }
            case BIG_AMU_15:
            {
                UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_Change, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 15 * 1000);
                break;
            }
            case BIG_AMU_16:
            {
                UIFloatDraw(&UI_Bullet_speed[1], "sd4", UI_Graph_Change, 8, UI_Color_Purplish_red, 15, 2, 2, 270, 550, 16 * 1000);
                break;
            }
        }
        // Char_ReFresh(&referee_recv_info->referee_id, UI_State_dyn[1]);
        UIGraphRefresh(&referee_recv_info->referee_id, 1 ,UI_Bullet_speed[1]);
        _Interactive_data->Referee_Interactive_Flag.friction_measure_flag = 0;
    }
    // power
    if (_Interactive_data->Referee_Interactive_Flag.Power_flag == 1)
    {
        ...
        UILineDraw(&UI_Energy[2], "sd6", UI_Graph_Change, 8, UI_Color_Pink, 30, 720, 160, (uint32_t)750 + _Interactive_data->Chassis_Power_Data.chassis_power_mx * 500 / 80, 160);
        UIGraphRefresh(&referee_recv_info->referee_id, 2, UI_Energy[1], UI_Energy[2]);
...
static void UIChangeCheck(Referee_Interactive_info_t *_Interactive_data)
{
    ...
    if (_Interactive_data->Bullet_Speed != _Interactive_data->Bullet_last_Speed)
    {
        _Interactive_data->Referee_Interactive_Flag.friction_measure_flag = 1;
        _Interactive_data->Bullet_last_Speed = _Interactive_data->Bullet_Speed;
    }
